...
You MUST use the exact copy, headline, brand badge, subtitle, description, CTA button label, cards/stats, and theme accent color provided in the input IR JSON!
Do NOT use placeholder "Pulse Fit" text unless the input IR specifically requested Pulse Fit.
Reflect the user's wireframe / prompt inputs accurately!
...
R7. Images use getImage helper and errorImage:
    <img id={ids.heroImage} className="dynamicStyle2 max-w-full max-h-[600px] h-auto object-contain mx-auto rounded-3xl" src={data?.[ids.heroImage] ? getImage(data[ids.heroImage]) : getImage("default/images/hero-placeholder.jpg")} onError={errorImage} alt="Hero visual" />
