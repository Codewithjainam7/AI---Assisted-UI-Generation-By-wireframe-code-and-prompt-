/**
 * generateFallback(ir)
 * Domain-aware template generator that reconstructs the EXACT interface drawn in the wireframe.
 * For e-commerce/product wireframes (e.g. Cushy Fleece Hoodie), it builds the full product page:
 * top navbar, 2x2 product image gallery, color swatches, size selectors, Add to Cart, wishlist, and specs.
 * For landing pages, it builds the modern split-hero interface.
 * Satisfies ALL contract rules R1-R14.
 */
export function generateFallback(ir) {
  ...
  const isEcommerce = ir.domain === 'ecommerce' ||
    ...
    /hoodie|shop|store|product|overview/i.test(ir.sectionName || '');

  const statBadgesEl = (ir.elements || []).find(e => e.elementName === 'statBadges');
  const rawCards = statBadgesEl?.statCards || (isEcommerce ? [
  ...
  // Reliable, verified product images for 2x2 gallery (avoiding 404s)
  const productImages = [
...
const COLOR_SWATCHES = [
  { name: "Dark Grey Heather", image: "${productImages[0]}" },
  { name: "Oatmeal Grey", image: "${productImages[1]}" },
  { name: "Jet Black", image: "${productImages[2]}" },
  { name: "Navy Blue", image: "${productImages[3]}" },
  { name: "Crimson Red", image: "${productImages[0]}" },
];
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-4">
              ...
              <div className="group relative rounded-xl sm:rounded-2xl overflow-hidden bg-zinc-900 border border-white/10 aspect-square shadow-lg">
                <img
                  ...
                  src={data?.[ids.heroImage] ? getImage(data[ids.heroImage]) : getImage("${productImages[0]}")}
                  onError={errorImage}
...
`;
  ...
  // Standard Split-Hero fallback for general SaaS / Landing wireframes
